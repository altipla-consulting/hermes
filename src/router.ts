...
export class Router {
  ...
  navigate(url: string) {    
    ...
    nextTick(async () => {
      if (this.route.loadingComponent?.navigate) {
        try {
...
export function createRouter({ routes, transformers }: PluginOptions) {
  ...
  let router = new Router(route, routes, transformers)
  ...
  if (window.__VUE_HMR_RUNTIME__) {
    ...
    window.__VUE_HMR_RUNTIME__.reload = function(id: string, def: Component) {
      ...
      if (shouldNavigateHMR(stale, def)) {
        ...
        let active = routes.find(r => r.component?.__hmrId === id)
        if (active) {
